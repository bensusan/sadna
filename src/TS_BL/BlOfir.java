...
public class BlOfir {
	...
	public boolean purchaseCart(Cart c, int creditCardNumber,String buyerAddress){
	...
	//if there is a product in newcart which doesn't exist in c it will return false
	public boolean editCart(Cart c, Map<Product,Integer> newCart){
		Map <Product,Integer> toRet = c.getProducts();
		for (Product p : newCart.keySet()){
			if(!c.getProducts().containsKey(p)){
				return false;					//the p product doesn't exist in the cart, therefore you need to add it
			}
