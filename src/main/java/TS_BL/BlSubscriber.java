...
public class BlSubscriber {
	...
	/**
	 * subscriber try to open a new store the address of the store will be the
	 * address of the subscriber the phone of the store will be the phone of the
	 * subscriber the id of the store will choose by the system
	 * 
	 * @param sub
	 * @param grading
	 * @param products
...
//		List<StoreOwner> own = sub.getOwner();
		...
		s = dalRef.getStoreByStoreId(s.getStoreId());
		sub = dalRef.getSubscriber(sub.getUsername(), sub.getPassword());
		return s;
	}
	...
	static boolean addPurchaseToHistory(Subscriber sub, Purchase p) throws Exception {
		...
		if (sub == null)
			throw new Exception("invalid subscriber");
		sub = dalRef.getSubscriber(sub.getUsername(), sub.getPassword());
		dalRef.addPurchaseToHistory(sub, p);
		sub = dalRef.getSubscriber(sub.getUsername(), sub.getPassword());
		return true;
	}
