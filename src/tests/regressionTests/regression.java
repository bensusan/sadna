...
public class regression {
	...
	{
		List <Subscriber> allSubscribers=BlMain.allSubscribers;
		assertTrue(allSubscribers.contains(ofir));
		assertTrue(allSubscribers.contains(or));
		assertTrue(allSubscribers.contains(sagiv));
		assertFalse(allSubscribers.contains(alex));
	...
	{
		int beforeadd=ofirStore.getProducts().get(tennisProduct).intValue();
		BlMain.addProductToCart(alex, tennisProduct, 10);
		assertEquals(ofirStore.getProducts().get(tennisProduct).intValue(),beforeadd);
	}
	

	@Test
	public void testPruchaseCart() {
		int beforebuy=ofirStore.getProducts().get(tennisProduct).intValue();
		BlMain.addProductToCart(alex, tennisProduct, 10);
		BlMain.purchaseCart(alex, "1111111111111111", "max shein 22 nes ziona");
		assertTrue(alex.getCart().getProducts().isEmpty());
		assertEquals(ofirStore.getProducts().get(tennisProduct).intValue(),beforebuy-10);
	...
	public void testPurchaseProduct() {
		int beforebuy=orStore.getProducts().get(golfProduct).intValue();
		BlMain.pruchaseProduct(ofir, golfProduct, 10, ((Subscriber)ofir).getCreditCardNumber(), ((Subscriber)ofir).getAddress());
		assertEquals(orStore.getProducts().get(golfProduct).intValue(),beforebuy-10);
		assertEquals(BlMain.getAllStoresWithThierProductsAndAmounts().get(orStore).get(golfProduct).intValue(),beforebuy-10);
